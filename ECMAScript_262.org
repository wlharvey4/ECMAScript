...
JavaScript is more than just what is defined in ECMA-262. A complete JavaScript
implementation is made up of the following three distinct parts:
...
#+cindex:ECMAScript
#+cindex:ECMA-262
EMCAScript  is  the language  defined  in  ECMA-262, and  is  not  tied to  web
browsers. The language has no methods  for input or output whatsoever. ECMA-262
defines the language  as a base upon which more  robust scripting languages may
be built. Web  browsers are just one /host environment/  in which an ECMAScript
implementation may exist.

#+cindex:host environment
#+cindex:extensions
A  host  environment  provides  the   base  implementation  of  ECMAScript  and
implementation extensions designed to to interface with the environment itself.

#+cindex:DOM
#+cindex:Document Object Model (DOM)
Extensions,  such as  the Document  Object Model  (DOM), use  ECMAScript's core
types and syntax  to provide additional functionality that is  more specific to
the  environment.  Other  host   environments  invlude  NodeJS,  a  server-side
JavaScript platform, and the obsolete Adobe Flash.

#+cindex:ECMA-262 specification
#+cindex:specification
EMCAScript is simply  a description of a language implementation  of all of the
facets  described  in  the  specification.  On a  very  basic  level,  ECMA-262
...
** ECMAScript Editions
#+cindex:editions of ECMAScript
The different  versions of EMCAScript  are defined as /editions/,  referring to
the edition of  ECMA-262 in which that particular  implementation is described.
The first edition of ECMA-262 was essentially the same as Netscape's JavaScript
1.1 but  with all references to  browser-specific code removed and  a few minor
changes. It also required support for  the Unicode standard to support multiple
languages and that objects be  platform-independent. JavaScript 1.1 and 1.2 did
/not/ conform to the ECMA-262 standard.

The second edition of ECMA-262 was  largely editorial. The standard was updated
to  conform with  ISO/IEC-16262  and  featured no  new  additions, changes,  or
omissions.

The third edition of ECMA-262 was the first real update to the standard.  It
provided updates to:
- string handling
- the definition of errors
- numeric outputs
- added support for regular expressions
- new controls statements
- ~try-catch~ exception handling
- small changes to better prepare the standard for internationalization

ECMAScript had arrived as a true programming language.

Ecma  TC39  convened to  decide  the  future  of  the language.  The  resulting
specification  defined an  almost completely  new language  based on  the third
edition.  As an  alternate proposal,  a specification  call ECMAScript  3.1 was
developed as a smaller evolution of the  language by a subcommittee of TC39 who
believed that the  fourth edition was too  big of a jump for  the language. The
result was a  small proposal with incremental changes to  ECMAScript that could
be implemented  on top of existing  JavaScript engines.

The fourth  edition was  abandoned in  favor of  ECMAScript 3.1.,  which became
ECMA-262 fifth edition, and was published on December 3, 2009.  This edition
sought to clarify perceived ambiguitities of the third and edition and
introduced additional functionality, which included:

- a native JSON object for parsing and serializing JSON data
- methods for inheritance and advanced property definition
- the inclusion of a new strict mode that slightly augments how ECMAScript
  engines interpret and execute code

A  maintenance edition  was  published in  2011 that  made  corrections in  the
specification but introduced no new language or library features.

The sixth  edition of ECMA-262, published  in 2015, is referred  to by multiple
names:
- ES6
- ES2015
- ES Harmony

This edition adds formal support for:
- classes
- modules
- iterators
- generators
...
The seventh edition of ECMA-262, named:
- ES7
- ES2016

was published in June 2016.  It included some syntactical additions such as:
- Array.prototype.includes
- exponentiation operator

The eigth edition of ECMA-26, called
- ES8
- ES2017

was finalized in January 2017, and included:
- asynchronous iteration
- rest and spread parameters
- new regular expression features
- Promise finally() catchall handler
- template literal revisions.


** ECMAScript Conformance
ECMA-262 defines ECMAScript conformance as an implementation that:
- supports all "types, values, objects, properties, functions, and program
  syntax and semantics" as they are described in ECMA-262;
- support the Unicode character standard

A conforming implementation may do the following:
- Add "additional types, values, objects, properties, and functions" that are
  not specified in ECMA-262.
- Support "program and regular expression syntax" that isnot defined in
  ECMA-262.
...
for XML that was extended for use in HTML. The DOM maps out an entire page as a
hierarchy  of nodes.  Each part  of  an HTML  or XML  page  is a  type of  node
containing different kinds of data. By creating a tree to represent a document,
DOM allows developers  an unprecendented level of control over  its content and
structure. Nodes can be removed, added,  replaced, and modified easily by using
the DOM API.
...
modules:
- the DOM Core, providing  a way to map the structure  of an XML-based document
  to allow for each assess to and manipulation of any part of a document; and
- the DOM  HTML, extending  the DOM  Core by  adding HTML-specific  objects and
  methods.

Whereas the goal of DOM Level 1 was to map out the structure of a document, the
DOM Level 2 were much broader.  This extension of the original DOM added
support for:
- mouse and user-interface events,
- ranges,
- traversals (methods to iterate over a DOM document)
- support for Cascading Style Sheets (CSS) through object interfaces

The original DOM Core was extended to include support for XML namespaces.

{{{heading(DOM Level 2)}}}

#+cindex:DOM Level 2
*DOM Level 2* introduced the following new modules of the DOM to deal with new
types of interfaces:

- DOM view :: describes interfaces to keep track of the various views of a
  document (the document before and after CSS styling, e.g.)
- DOM events :: describes interfaces for events and event handling
- DOM style :: describes interaces to deal with CSS-based styling of elements
- DOM traversal and  range :: describes  interaces to traverse and manipulate a
  document tree
...
and save documents in a uniform way  (contained in a new module called DOM Load
and Save) and methods to validate a  document (DOM Validation). The DOM Core is
extended to support all of XML 1.0, including XML Infoset, XPath, and XML Base.

{{{heading(DOM Living Standard)}}}

#+cindex:DOM Living Standard
Now the  W3C no longer  maintains the DOM  as a set of  levels, but as  the DOM
Living Standard.

*** DOM Support in Web Browsers
The  DOM  had  been a  standard  for  some  time  before web  browsers  started
implementing it. Now, most browsers support  all of Level 1, partially Levels 2
and 3.

** The Browser Object Model---BOM
The BOM deals with the browser window and frames; generally any
browser-specific extension to JavaScript is consdered to be a part of the BOM.
...
- the =location= object, which gives detailed information about the page loaded
  in the browser
- the =screen= object, which gives detailed information about the browser's
  memory consumption, navigational behaviro, and timing statistics
- support for cookies
- custom objects such as ~XMLHttpRequest~ and ~ActiveObject~

Becauseno standards existed for  the BOM for a long time,  each browser has its
own  implementation. There  are some  /de facto/  standards, such  as having  a
=window= object  and a  =navigator= object,  but each  browser defines  its own
properties  and methods  for  these and  other objects.  HTML5  is supposed  to
provide implementation details over time.
...
** ECMAScript History
   ...
   - 2016 ::
     ...
     - first ECMAScript edition released under Ecma TC39's new yearly release
       cadence and open development process
     - new exponentiation operator
     - adds a new method to ~Array.prototype~ called ~includes~
     - Additionally, numerous software tools were developed to aid in this
       effort including
       - Ecmarkup
       - Ecmarkdown
       - Grammarkdown
   ...
   - 2019 ::
     ...
     - introduces a few new built-in functions:
       ...
       - ~Object.fromEntries~ for directly turning the return value of
         ~Object.entries~ into a new Object
       - ~String.prototype~
         - ~trimStart~
         - ~trimEnd~
         - to replace
       ...
       - a few minor updates to syntax and semantics
         ...
         - allowing U+2028 (LINE SEPARATOR) and U+2029 (PARAGRAPH SEPARATOR) in
           string literals to align with JSON
       - requiring that ~Array.prototype.sort~ be a stable sort
       - requiring that ~JSON.stringify~ return well-formed UTF-8 regardless of
         input
       - clarifying ~Function.prototype.toString~ by requiring that it either
         return the corresponding original source text or a standard
         placeholder.
...
** Edition 10 --- ES2019
   ...
   June 2019.  [[http://www.ecma-international.org/publications/standards/Ecma-262.htm][ECMAScript 2019]]

   This Standard defines the ECMAScript 2019 general-purpose programming
   language.

   - [[http://www.ecma-international.org/ecma-262/10.0/index.html#Title][Browsable HTML]]
   - [[http://www.ecma-international.org/publications/files/ECMA-ST/ECMA-262.pdf][PDF File]]

** Edition 11 -- ES2020

[[https://tc39.es/ecma262/2020/][ECMAScript 2020]]

- GitHub Repo :: https://github.com/tc39/ecma262

- Introduction ::

                  This Ecma Standard defines the ECMAScript 2020 Language.  It
                  is the eleventh edition of the ECMAScript Language
                  Specification.  Since publication of the first edition in
                  1997, ECMAScript has grown to be one of the world's most
                  widely used general-purpose programming languages.  It is
                  best known as the language embedded in web browsers but has
                  also been widely adopted for server and embedded
                  applications.

** Edition 12 -- ES2021 (Draft)
[[https://tc39.es/ecma262/][ECMAScript® 2021 Language Specification]]
...
- About ::
           ...
           most recent yearly snapshot plus any finished proposals (those that
           have reached Stage 4 in the proposal process and thus are
           implemented in several implementations and will be in the next
           practical revision) since that snapshot was taken.

- Introduction ::

                  This Ecma Standard defines the ECMAScript 2021 Language.  It
                  is the twelfth edition of the ECMAScript Language
                  Specification.  Since publication of the first edition in
                  1997, ECMAScript has grown to be one of the world's most
                  widely used general-purpose programming languages.  It is
                  best known as the language embedded in web browsers but has
...
- [[https://github.com/tc39/ecma262/][on GitHub]] and [[https://github.com/tc39][here]]
...
*** Stage 3 Proposals (current as of 2020-04-18)
- [[https://github.com/tc39/proposal-regexp-legacy-features][Legacy RegExp features in JavaScript]]
- [[https://github.com/tc39/proposal-import-meta][import.meta]]
- [[https://github.com/tc39/proposal-private-methods][Private instance methods and accessors]]
...
#+cindex:hard binding with pass-through
...
#+cindex:@code{bind} helper
Another way to express this pattern is to create a reusable helper:

#+caption:Hard Binding with Reusable Helper
#+name:hard-binding-reusable-helper
#+begin_src js -n
  function foo(something) {
      console.log( this.a, something );
      return this.a + something;
...
#+cindex:@code{Function.prototype.bind} method
#+cindex:ECMAScript 5
Since hard binding is such a common pattern, it’s provided with a built-in
utility as of ES5, ~Function.prototype.bind~, and it’s used like this:

#+caption:Function Prototoype Bind
...
operator in front of them.  They are not attached to classes, nor are they
instantiating a class.  They are not even special types of functions.  They’re
just regular functions that are, in essence, hijacked by the use of ~new~ in
their invocation.

Any function, including the built-in object functions like ~Number(..)~ can be
called with ~new~ in front of it, and that makes that function call a
constructor call.  There’s really no such thing as “constructor functions,”
but rather construction calls of functions.

***** Constructor Calls with ~new~

#+cindex:constructor call
...
and set that new object as the ~this~ for the call of ~foo(..)~.  So ~new~ is
the final way that a function call’s ~this~ can be bound.  We’ll call this
/new/ binding.
...
*** New Array Methods
**** ArrayBuffer Objects
#+cindex:@code{ArrayBuffer} object
...
languages as a "byte array".

You cannot directly manipulate the contents of an =ArrayBuffer=; instead, you
create one of the typed array objects or a =DataView= object which represents
the buffer in a specific format, and use that to read and write the contents of
the buffer.

{{{heading(ArrayBuffer Syntax)}}}

#+attr_texinfo: :options Constructor {new ArrayBuffer} ( LENGTH )
#+begin_deffn
Create a new =ArrayBuffer= object of size {{{var(length)}}}.

- {{{var(length)}}} :: The size, in bytes, of the array buffer to create.

- Return Value :: A new ArrayBuffer object of the specified size. Its contents
                  are initialized to 0.

- Exceptions :: A RangeError is thrown if the length is larger than
                =Number.MAX_SAFE_INTEGER= or negative
#+end_deffn

#+cindex:structured data
- 24. ECMA-262 6th Edition Structured Data
  #+cindex:@code{ArrayBuffer} objects
- 24.1. ECMA-262 6th Edition =ArrayBuffer= Objects
- 24.1.1. Abstract Operations For =ArrayBuffer= Objects
  #+cindex:@code{ArrayBuffer} constructor
- 24.1.2. The =ArrayBuffer= Constructor

  The =ArrayBuffer= constructor is the =%ArrayBuffer%= intrinsic object and the
  initial value of the =ArrayBuffer= property of the global object.  When
  ...
  The =ArrayBuffer= constructor is designed to be subclassable.  It may be used
...
- 24.1.3. Properties of the =ArrayBuffer= Constructor
  ...
  #+cindex:@code{ArrayBuffer.isView()}
  - ArrayBuffer.isView ( arg )

    #+cindex:@code{ArrayBuffer.prototype}
  - ArrayBuffer.prototype

    This property has the attributes { [[Writable]]: false, [[Enumerable]]: false,
    [[Configurable]]: false }.

    #+cindex:@code{get ArrayBuffer}
...
- 24.1.4. Properties of the =ArrayBuffer= =Prototype= Object
  ...
  #+cindex:@code{ArrayBuffer.byteLength}
  - get ArrayBuffer.prototype.byteLength

    #+cindex:@code{ArrayBuffer.constructor}
  - ArrayBuffer.prototype.constructor

    #+cindex:@code{ArrayBuffer.slice()}
...
- 24.1.5. Properties of the =ArrayBuffer= Instances
  ...
  =ArrayBuffer= instances each have:

  #+cindex:@code{ArrayBuffer} Data
...
=ArrayBuffer= instances whose =[ [ArrayBufferData] ]= is =null= are considered to
be detached and all operators to access or modify data contained in the
=ArrayBuffer= instance will fail.
**** DataView Objects
#+cindex:@code{DataView}
#+cindex:endianness
...
the platform's endianness.  =DataView= accessors provide explicit control of
how data is accessed, regardless of the executing computer's endianness.

**** TypedArray Objects
#+cindex:@code{TypedArray} object
A =TypedArray= object describes an array-like view of an underlying binary data
buffer (=ArrayBuffer=).
...
constructor argument, then this is used instead.  The buffer address is saved
as an internal property of the instance and all the methods of
=%TypedArray%.prototype=, i.e. ~set~ value and ~get~ value etc., operate on
that array buffer address.

There are a number of different global properties, whose values are typed array
constructors for specific element types.

A =TypedArray= can be one of:
...
- length :: an internal array buffer is created in memory, of size length
            multiplied by BYTES_PER_ELEMENT bytes, containing zeros.

- typedArray :: When called with a =typedArray= argument, which can be an
                object of any of the typed array types (such as =Int32Array=),
                the =typedArray= gets copied into a new typed array.  Each
                value in =typedArray= is converted to the corresponding type of
                the constructor before being copied into the new array.  The
                length of the new typed array will be same as the length of the
                =typedArray= argument.

- object :: When called with an object argument, a new typed array is created
            as if by the ~TypedArray.from()~ method.

- buffer, byteOffset, length :: When called with a buffer, and optionally a
...
{{{heading(Buffers and Views)}}}

To achieve maximum flexibility and efficiency, JavaScript typed arrays split
the implementation into buffers and views.

{{{subheading(Buffers)}}}

A buffer (implemented by the ArrayBuffer object) is an object representing a
chunk of data; it has no format to speak of and offers no mechanism for
accessing its contents. In order to access the memory contained in a buffer,
...
** Modules
ES6 modules are a good example.  Other languages have module systems.  Racket
has a great one.  Python too. When the standard committee decided to add
modules to ES6, why didn’t they just copy an existing system?

JS is different, because it runs in web browsers.  I/O can take a long time.
Therefore JS needs a module system that can support loading code
asynchronously.  It can’t afford to serially search for modules in multiple
directories, either.  Copying existing systems was no good.  The ES6 module
system would need to do some new things.
...
:url:      https://www.andyshora.com/learning-javascript-resources-2019
:END:

** Interactive Learning
- [[https://www.codecademy.com/learn/javascript][Codeacademy]] :: Fundamentals of JavaScript.
- [[https://www.udacity.com/course/javascript-basics--ud804][Udacity JavaScript Basics]] :: They also offer more advanced courses.
- [[https://www.codewars.com/][Code Wars]] :: Improve your skills by training with others on real code
     challenges.
- [[https://www.codeschool.com/learn/javascript][Code School]] :: From Basics to Best Practices. Different courses related to
     JavaScript.
- [[https://exercism.io/][Exercism]] :: Code practice and mentorship for everyone.
- [[https://hackr.io/][hackr.io]] :: Find the Best Programming Courses & Tutorials.

** Web Development Playgrounds
- [[http://jsbin.com/][JSBin]] :: One of the most popular web development playgrounds.
- [[https://jsfiddle.net/][JSFiddle]] :: JSFiddle is a lightweight cloud JS editor designed for developers
     who want to tweak and test blocks of JS, CSS and HTML code.
- [[http://codiad.com/][Codiad]] :: Codiad is a web-based IDE framework with a small footprint and
     minimal requirements.
- [[http://plnkr.co/][Plunker]] :: Plunker is an online community for creating, collaborating on and
     sharing your web development ideas.
- [[https://codepen.io/][Codepen]] :: a playground for the more creative people.  Pens often include
     inspirational CSS, Canvas and WebGL examples.

** Free EBooks
- [[https://leanpub.com/understandinges6/read/][Understanding ECMAScript 6]] :: Provides a great understanding of ECMAScript 6
     features.
- [[http://exploringjs.com/es2016-es2017.html][Exploring ES2016 and ES2017]] :: This book covers the latest versions of
     JavaScript as they are created.
- [[http://exploringjs.com/es6/][Exploring ES6]] :: An in-depth book on ECMAScript 6, for JavaScript
     programmers.
- [[https://goalkicker.com/JavaScriptBook/][JavaScript® Notes for Professionals]] :: Compiled from Stack Overflow
     Documentation, the content is written by the beautiful people at Stack
     Overflow.
- [[https://github.com/bpesquet/thejsway][The JavaScript Way book]] :: A modern introduction to an essential language.
- [[https://ponyfoo.com/books/practical-modern-javascript][Practical Modern JavaScript]] :: A practical book which shows you how to build
     small, interconnected ES6 JavaScript modules that emphasise reusability.
- [[https://gitbookio.gitbooks.io/javascript/content/][Learn Javascript]] :: This book will teach you the basics of programming and
     Javascript, with lovely embedded interactivity.
- [[http://eloquentjavascript.net/][Eloquent JavaScript]] :: Covering the language and runtime specifics.
- [[https://github.com/getify/You-Dont-Know-JS][You Don’t Know JS]] (book series) :: Series of books diving deep into language.
- [[http://speakingjs.com/][Speaking JavaScript]] :: In-depth guide beginning with the basics.
- [[http://addyosmani.com/resources/essentialjsdesignpatterns/book/][JavaScript Design Patterns]] :: Classical and JavaScript specific design
     patterns.
- [[https://mostly-adequate.gitbooks.io/mostly-adequate-guide/][Mostly Adequate Guide to Functional Programming]] :: Excellent primer to
     functional programming using JavaScript.

** Videos
- [[https://www.youtube.com/channel/UCO1cgjhGzsSYb1rsB4bFe4Q][FunFunFunction]] :: Educational plus entertaining YouTube show covering
     language features as well as architectural topics amongst others.
- [[http://latentflip.com/loupe/?code=!!!PGJ1dHRvbj5DbGljayBtZSE8L2J1dHRvbj4%253D][What the heck is the event loop anyway?]] :: Awesome talk about the way
     JavaScript works, and a tool for exploring the callstack.
- [[https://www.youtube.com/watch?v=4mf_yNLlgic][Become a Javascript Console Power-User]] :: Introduction to the browsers
     JavaScript console.
- [[https://www.youtube.com/watch?v=HF1luRD4Qmk][Debugging The Web]] :: Learn state of the art in debugging using Chrome dev
     tools.
- [[https://www.youtube.com/watch?v=qGyqzN0bjhc][== ? === ??? …#@^%]] :: Basic talk about type coercion and strict type
     comparison.
- [[https://vimeo.com/181328943][Promises Are So Passé]] :: Talk that shows where async goes next, why it
     matters, and what you need to do to put it into practice today.
- [[https://www.youtube.com/watch?v=e-5obm1G_FY][Learning Functional Programming with JavaScript]] :: Talk containing the best
     explanation of map/reduce.
- [[https://egghead.io/courses/understand-javascript-s-this-keyword-in-depth][Understand JavaScript’s this Keyword in Depth]] :: 18 minutes divided into 8
     short videos to wrap you head around this.

** ES6 and Above
- [[https://devhints.io/es6][ES2015+ cheatsheet]] :: A quick overview of new JavaScript features in ES2015,
     ES2016, ES2017, ES2018 and beyond.
- [[https://github.com/DrkSephy/es6-cheatsheet][ES2015 {ES6} cheatsheet containing tips, tricks, best practices and code snippets]] :: Another
     excellent cheatsheet, also partnered with with slides.
- [[http://exploringjs.com/setting-up-es6.html][Setting up ES6]] :: Explains how to set up ES6 projects that are transpiled to
     ES5.
- [[https://ponyfoo.com/articles/es6][ES6 Overview]] :: Bullet point overview including in-depth articles.
- [[http://es6katas.org/][ES6 Katas]] :: Learn ES6 by solving unit test online.
- [[https://github.com/mjavascript/practical-es6][Practical ES6]] :: eBook with practical examples and advices.
- [[https://github.com/Chalarangelo/30-seconds-of-code][30 seconds of code]] :: Useful ES6 snippets that you can understand in 30
     seconds or less.

** DOM Stuff
- [[http://domenlightenment.com/][DOM Enlightenment]] :: A whole book about how to access an manipulate the DOM
     without a library.
- [[http://youmightnotneedjquery.com/][You Might Not Need jQuery]] :: Get plain JavaScript code snippets (and see
     their jQuery equivalents).
- [[https://zellwk.com/blog/dom-traversals/][Traversing the DOM with JavaScript]] :: A tutorial featuring native methods to
     traverse the DOM.

** JavaScript References
- [[https://developer.mozilla.org/docs/Web/JavaScript/Reference][MDN]] :: Simply the best language reference.
- [[http://devdocs.io/javascript][DevDocs]] :: Search MDN comfortably. Even offline.
- [[http://jargon.js.org/][Simplified JavaScript Jargon]] :: Glossary which explains all the buzzwords
     from the JavaScript eco system.
- [[https://functional.works-hub.com/blog/Functional-Programming-Jargon][Functional Programming Jargon]] :: Explains terms used in functional
     programming in the JavaScript context.
- [[https://github.com/mbeaudru/modern-js-cheatsheet][Modern JavaScript Cheatsheet]] :: Helps developers with basic knowledge to get
     familiar with modern codebases.
- [[http://ecma-international.org/publications/standards/Ecma-262.htm][ECMAScript® Language Specification]] :: The standard JavaScript is based
     on. Only for very advanced learners.

** Node.js
- [[https://github.com/maxogden/art-of-node#readme][The Art of Node]] :: Introductory tutorial covering the basics.
- [[https://nodeschool.io/][NodeSchool]] :: Interactive self guided workshops you can also do on your own.
- [[http://nodepatternsbooks.com/][Node Patterns]] :: Short books about code and networking patterns related to
     Node.js.
- [[https://learnnode.com/][Learn Node]] :: A premium training course to learn to build apps with Node.js,
     Express, MongoDB.
